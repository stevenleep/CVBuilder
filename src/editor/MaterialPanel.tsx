...
export const MaterialPanel: React.FC = () => {
  ...
  const filteredMaterials = allMaterials.filter(material => {
    ...
    if (activeCategory !== 'all' && activeCategory !== 'templates' && material.meta.category !== activeCategory) {
      return false
    }
  ...
  const categoryList = [
    ...
    ...categories.map(cat => ({ id: cat, name: getCategoryName(cat) }))?. filter(cat => cat.id !== 'system'),
  ]

  const handleAddMaterial = (materialType: string) => {
    const parentId = selectedNodeIds.length > 0 
      ? selectedNodeIds[0] 
      : pageSchema.root.id
  ...
  const handleAddTemplate = (template: CustomTemplate) => {
    ...
    store.setPageSchema({
      ...
      root: {
        ...
        children: [
          ...(store.pageSchema.root.children || []),
          clonedNode,
        ],
      },
    })
